...
class Card extends Component {
  ...
  getColor = (isEmph) => {
    ...
    var selectedSkillLevel = 0;

    if (isEmph == true) {

      for (let i in this.props.elem.skills) {
        if (this.props.elem.skills[i].name === this.props.selectedSkill) {
          selectedSkillLevel = this.props.elem.skills[i].level
        }
      }
    }

    selectedSkillLevel = Math.floor(selectedSkillLevel);
    ...
    return (
      ...
      >
        <Header
          ...
          isEmph={this.getEmph()}
          color={this.getColor(this.getEmph())}
        />
